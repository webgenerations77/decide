...
const GEOAPIFY_KEY   = process.env.EXPO_PUBLIC_GEOAPIFY_API_KEY; // eslint-disable-line no-unused-vars
...
// ─── PRIORITY 3: Weather with wind + emoji ────────────────────────────────────
...
// ─── PRIORITY 2: Reverse geocode for NPS city/state ──────────────────────────

async function reverseGeocode(lat, lng) {
  if (!GOOGLE_KEY || GOOGLE_KEY === 'your_api_key_here') return { city: null, state: null };
  try {
    const base = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${GOOGLE_KEY}`;
...
// ─── PRIORITY 2: RIDB recreation areas ───────────────────────────────────────
...
// ─── PRIORITY 2: Enrich itinerary with OpenRoute driving times ───────────────
...
// Builds an itinerary from already-fetched Google/NPS/RIDB place data.
// Used when Claude is unavailable (rate limited, out of tokens, no key, etc.)
function buildFallbackItinerary({ food, activity, shopping, outdoor, startTime, endTime, pace, lat, lng }) {
  ...
  // Category sequences by stop count
  const SEQUENCES = {
  ...
  for (const cat of template) {
    ...
    current += duration + 20; // travel buffer
  }

  return stops;
}

// ─── PRIORITY 1: callClaude with optional web search ─────────────────────────

async function callClaude(systemPrompt, userPrompt, options = {}) {
  const { maxTokens = 2048, useWebSearch = false } =
  ...
  for (let attempt = 0; attempt < 6; attempt++) {
    ...
    if (data.stop_reason !== 'tool_use') return textBlock?.text ?? '';
    // Continue the agentic loop for server-side tool calls
    messages.push({ role: 'assistant', content: data.content });
    const toolCalls = (data.content ?? []).filter((b) => b.type === 'tool_use');
    if (toolCalls.length === 0) return textBlock?.text ?? '';
    messages.push({
      role: 'user',
      content: toolCalls.map((b) => ({ type: 'tool_result', tool_use_id: b.id, content: '' })),
    });
  }
  return '';
}
...
export async function generateItinerary({
  ...
  // Parallel: Google Places + weather + reverse geocode
  ...
  // Parallel: NPS + RIDB (depend on geo result)
...
Return a JSON array only. Each element must have exactly these fields:
{
  ...
  try {
    const raw    = await callClaude(systemPrompt, userPrompt, { maxTokens: 4096, useWebSearch: true });
  ...
  // Enrich with real driving times if OpenRoute key is configured
...
export async function swapStop({ itinerary, stopIndex, latitude, longitude }) {
  ...
  if (available.length === 0) {
    throw new Error(`No alternative ${stop.category} places found nearby`);
  }

  const systemPrompt =
    'You are a day planner. You return ONLY a valid JSON object for a single itinerary stop. ' +
    'No prose, no markdown, no explanation.';

  const userPrompt = `Replace this itinerary stop with a different option.
...
- Pick a DIFFERENT place from the available list — not the same name as the stop being replaced
- Write a fresh "reason" for the new pick
- Return a single JSON object (not an array) with fields: time, duration_mins, category, name, place_id, address, lat, lng, reason, excitement_score`;

  let newStop;
  try {
    const raw    = await callClaude(systemPrompt, userPrompt, { maxTokens: 512 });
    const parsed = JSON.parse(extractJSON(raw));
    const candidate = Array.isArray(parsed) ? parsed[0] : parsed;
    if (!candidate?.name) throw new Error('Invalid stop returned');
    newStop = candidate;
  } catch (e) {
    console.warn('[itinerary] Claude swap unavailable, picking locally:', e.message);
    const pick = available[0];
    if (!pick) throw new Error(`No alternative ${stop.category} places found nearby`);
    newStop = {
      time:            stop.time,
