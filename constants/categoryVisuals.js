...
// Helper: match key as whole word within the string (word-boundary regex).
function matches(s, k) {
  const esc = k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`\\b${esc}\\b`).test(s);
}

// Ordered — first match wins; put more specific rows before generic ones.
// Each row: match if the normalized category EQUALS or is a whole-word MATCH for any key.
const ROWS = [
  ...
  { keys: ['spa', 'wellness', 'relax', 'massage', 'yoga', 'sauna', 'retreat'],           icon: 'flower',         color: COLORS.beta },
...
export function categoryVisual(raw) {
  ...
  for (const row of ROWS) {
    if (row.keys.some((k) => s === k || matches(s, k))) return { icon: row.icon, color: row.color };
  }
  return DEFAULT;
}
