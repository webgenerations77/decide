 ...
 *   1. Strips the eleven voiceover tracks (35% of the file). Browsers block autoplay audio
 *      anyway, and a loading screen that starts talking is hostile.
 *   2. Never calls Music.init(). Every method of the synth score guards on `ac`/`master`, so
 *      skipping init leaves the whole engine inert — no AudioContext, no console warnings.
...
async function main() {
  ...
  // ── 1. Strip the voiceover. `VO` becomes empty, so playVO's `!c` guard returns early and
  //       the <audio> element is never given a src.
  html = mustReplace(
    html,
    /const VO = \{open:[\s\S]*?\};(?=\s*\/\/ \{id:\{d,src\}\})/,
    'const VO = {}; /* EMBED: voiceover stripped — autoplay audio is blocked and unwanted here */',
    'the VO (voiceover) table',
  );
  ...
  // ── 4. Start muted. Belt-and-braces: VO is already empty and Music is never initialised.
  html = mustReplace(html, /muted=false, started=false/, 'muted=true, started=false',
    'the muted/started state flags');
...
<script>`,
  ...
  /* ══ EMBED MODE — generated by scripts/build-house-ad.js, do not hand-edit ══
     ...
     Music.init() is deliberately NEVER called: every method of the score guards on \`ac\`,
     so leaving it uninitialised makes the whole audio engine a silent no-op. */
