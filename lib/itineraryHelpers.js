...
// Map a budget to the allowed Places (New) v1 priceLevel enums (1..selected).
// NOTE: intentionally NOT sent to Nearby Search — see the comment there. Kept so the
// mapping lives in one place if we ever move budget enforcement to the API layer.
const PRICE_LEVEL_ENUMS = ['PRICE_LEVEL_INEXPENSIVE', 'PRICE_LEVEL_MODERATE', 'PRICE_LEVEL_EXPENSIVE', 'PRICE_LEVEL_VERY_EXPENSIVE'];
export function budgetToPriceLevels(budget) {
  return PRICE_LEVEL_ENUMS.slice(0, budgetToPriceLevel(budget));
}

// Compact "HH:MM-HH:MM" open-hours string for `weekday` (0=Sun..6=Sat) from a Places
// (New) v1 openingPeriods array (regularOpeningHours/currentOpeningHours .periods, each
// period = { open:{day,hour,minute}, close:{day,hour,minute} }). Returns null when hours
// are unknown, "closed" when the venue has hours but none that weekday, "open 24h" for
// round-the-clock venues.
export function compactHours(periods, weekday) {
  if (!Array.isArray(periods) || !periods.length) return null;
  // 24h venues: a single open period with no close.
  if (periods.length === 1 && periods[0]?.open && !periods[0]?.close) return 'open 24h';
  const fmt = (t) => `${String(t?.hour ?? 0).padStart(2, '0')}:${String(t?.minute ?? 0).padStart(2, '0')}`;
  const today = periods.filter((pd) => pd?.open?.day === weekday);
  if (!today.length) return 'closed';
  return today.map((pd) => `${fmt(pd.open)}-${pd.close ? fmt(pd.close) : '24:00'}`).join(', ');
}

// Copy of a Google place with a compact `hours` string for the plan weekday, stripping the
// bulky raw periods so they never bloat the synthesis prompt. Places with no hours data
// (parks, NPS/RIDB, etc.) come back without an `hours` field (i.e. "unknown").
export function withHours(place, weekday) {
  if (!place) return place;
  const { openingPeriods, ...rest } = place;
  const hours = compactHours(openingPeriods, weekday);
  return hours ? { ...rest, hours } : rest;
}

// Prefer a client-supplied locationLabel ("Berlin, MD") for locality context. Parses a
// best-effort { city, state } from it (for NPS / state-scoped lookups) without a geocode call.
export function parseLocationLabel(label, short) {
  const parts = String(label || '').split(',').map((s) => s.trim()).filter(Boolean);
  const city = (short && String(short).trim()) || parts[0] || null;
...
export function pickForecastFromOpenMeteo(om, dateISO) {
  ...
  // Open-Meteo sunrise/sunset with timezone=auto are local ISO strings ("2026-07-22T05:52").
  const localTime = (arr) => {
    const v = arr?.[i];
    if (!v || typeof v !== 'string') return null;
    const t = v.split('T')[1];
    return t ? t.slice(0, 5) : null; // "HH:MM" local
  };
  return {
    ...
    sunrise: localTime(d.sunrise),
    sunset: localTime(d.sunset),
