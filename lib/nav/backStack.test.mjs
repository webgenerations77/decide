...
test('popstate landing on our own sentinel (foreign entry popped above it) does not dismiss a layer', async () => {
  const win = makeFakeWin();
  const bs = createBackStack(win);
  let dismissed = 0;
  bs.push(() => { dismissed++; });
  await tick();                                   // sentinel armed: [base, sentinel], index 1
  win.history.pushState({ route: '/paywall' });   // foreign entry ABOVE the sentinel, index 2
  win.history.back();                             // back to the sentinel: event.state.__backSentinel === true
  assert.equal(dismissed, 0);                     // must NOT dismiss the layer
  assert.equal(bs._state().depth, 1);
  assert.equal(bs._state().hasSentinel, true);
  win.history.back();                             // now consume our sentinel: land on base (not sentinel)
  assert.equal(dismissed, 1);                     // layer dismissed as normal
  assert.equal(bs._state().depth, 0);
});
