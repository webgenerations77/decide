...
export function createBackStack(win) {
  ...
  let pendingPop = false;      // true while a self-initiated back() is in flight
  let listening = false;
  ...
  function reconcile() {
    ...
    if (pendingPop) return; // a self-initiated back() is in flight; wait for its popstate
  ...
  function onPop(e) {
    if (pendingPop) {         // the popstate from our own sentinel-consuming back()
      pendingPop = false;
      scheduleReconcile();    // reconcile against the confirmed real history state
      return;
    }
    // Landed back ON our sentinel: a foreign entry pushed above it (router.push,
    // tab switch) was popped — not our sentinel. Don't dismiss a layer; the
    // sentinel is still the top entry.
    if (e && e.state && e.state.__backSentinel) return;
