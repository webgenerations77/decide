 ...
 *
 * Only ever called on demand (the leg-alternatives sheet), never during generation: this is
 * one computeRoutes query per call, and paying it for every leg of every itinerary is the
 * spend the ambiguous-band design exists to avoid.
 *
 * Returns null when there is no transit route, which is the common case outside metros.
 */
async function transitRoute(from, to) {
  if (!ROUTES_KEY) return null;
  try {
