...
export const LOCAL_TRANSPORT = [
  ...
  },
  {
    ...
    months: null,
    // A deliberate NEGATIVE entry: it warns, it is not a service. Anything offering these as
    // selectable options (see availability.js) must filter it out.
    unavailable: true,
...
export function getLocalTransport(lat, lng, date = null) {
  ...
  return LOCAL_TRANSPORT
    ...
    .map(({ id, name, text, url, region, unavailable }) => ({ id, name, text, url, region, unavailable: !!unavailable }));
}
