 ...
 */
export function legHintText(leg, verdict = null) {
  if (!leg) return null;
  const manner = HINT_MANNER[leg?.mode];
  if (!manner) return null;

  // ⚠ Don't repeat the day's call under every stop. On a "Drive it" day a real Delmarva plan
  // rendered "8.7 mi by car / 9.7 mi by car / 3.6 mi by car / 4.8 mi by car" beneath a verdict
  // that had already said "Drive it" — the module's own rule is that the mode is said ONCE, at
  // day level, never per leg. The distance is the new information; the manner is not, and the
  // chip's icon carries it anyway. Where the leg DIFFERS from the verdict the manner earns its
  // place (a walk on a transit day is worth naming), so it survives there.
  const echoesVerdict = !!verdict && leg.mode === verdict.mode;
  const miles = isNum(leg.miles) ? `${round1(Number(leg.miles))} mi` : null;

  if (!miles) return echoesVerdict ? null : manner.charAt(0).toUpperCase() + manner.slice(1);
  const body = echoesVerdict ? miles : `${miles} ${manner}`;
  return leg.mins ? `${body} · ${leg.mins} min` : body;
}
