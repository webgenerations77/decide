 ...
 */
export function dayVerdict(classified = [], { transit = 'unknown', gettingAround = 'car' } = {}) {
  ...
  } else if (transit === 'yes' && driveLegs.length && totalMiles < 15) {
    mode = 'transit';
  } else {
    mode = 'drive';
  }
  ...
  const detail = mode === 'drive' || mode === 'transit'
...
/**
 * Does this leg deserve a chip on the timeline?
 *
...
export function legChipText(leg) {
  ...
  const verb = leg.mode === 'walk' ? 'Walk it' : leg.mode === 'bike' ? 'Bike it' : 'Drive it';
