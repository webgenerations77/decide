...
test('validateStops mutual exclusivity: verified wins over unverified/time_note', () => {
  const [out] = validateStops([
    { ...baseStop, verified: true, verify_source: 'https://x.com/sched', unverified: true, time_note: 'confirm' },
  ]);
  assert.equal(out.verified, true);
...
test('validateStops guards: verified:true without verify_source is dropped, hedge fields survive', () => {
  const [out] = validateStops([
  ...
  assert.equal('verify_source' in out, false);
  assert.equal(out.unverified, true);
  assert.equal(out.time_note, 'confirm');
});
...
test('buildSynthesisPrompt: avoid list produces a HARD AVOID rule naming the places', () => {
  const { user } = buildSynthesisPrompt({
    places: [], finds: [], anchors: [],
    ctx: {
      location: 'Berlin, MD', startTime: '11:00', endTime: '20:00', prefs: {},
      feedback: { dislikedPlaces: ['Touristy Crab Shack', 'The Loud Bar'] },
    },
  });
  assert.match(user, /HARD AVOID/);
  assert.match(user, /Touristy Crab Shack/);
  assert.match(user, /The Loud Bar/);
});

test('buildSynthesisPrompt: no avoid list → no dangling HARD AVOID rule', () => {
  const { user } = buildSynthesisPrompt({
    places: [], finds: [], anchors: [],
    ctx: { location: 'Berlin, MD', startTime: '11:00', endTime: '20:00', prefs: {}, feedback: { dislikedPlaces: [] } },
  });
  assert.doesNotMatch(user, /HARD AVOID/);
});
