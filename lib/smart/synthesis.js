...
import { synthesisConstraint } from '../transport/gettingAround.js';
import { transitPrefConstraint } from '../transport/availability.js';
...
function findTime(f) {
  if (!f) return '';
  if (f.verifiedTime) return ` — VERIFIED start ${f.verifiedTime} (confirmed via source; use exactly, do not hedge)`;
  if (!f.startTime) return '';
  return ` — starts ${f.startTime}${f.timeConfidence ? ` (${f.timeConfidence} confidence)` : ''}`;
}

function verifySourceTag(f) {
  if (!f?.verifiedTime || !f?.verifiedSource) return '';
  return ` [verify_source: ${f.verifiedSource}]`;
}

export function buildSynthesisPrompt({ places, finds, anchors, ctx }) {
  const p = ctx.prefs || {};
  const sun = ctx.sun || {};
  const avoid = (ctx.feedback?.dislikedPlaces || []).filter(Boolean);
  const sunrise = sun.sunrise || 'unknown';
  const sunset = sun.sunset || 'unknown';
  const anchorBlock = anchors.length
    ? anchors.map((a, i) => `ANCHOR ${i + 1}: ${a.find.title} @ (${a.find.lat},${a.find.lng})${findTime(a.find)}${verifySourceTag(a.find)} — ${a.rationale} [${a.find.sourceLabel}]`).join('\n')
    : '(none — build from places and any finds below)';
  const findBlock = finds.slice(0, 20).map((f) => `- ${f.title}${f.when ? ` — ${f.when}` : ''}${findTime(f)}${verifySourceTag(f)}${f.snippet ? ` (${f.snippet})` : ''} @ (${f.lat},${f.lng}) [${f.interest}/${f.sourceLabel}]`).join('\n') || '(none)';
...
Rules:
${(() => { const c = synthesisConstraint(p.gettingAround); return c ? `- ${c}\n` : ''; })()}${(() => { const c = transitPrefConstraint({ id: p.transitPref, label: p.transitPrefLabel }); return c ? `- ${c}\n` : ''; })()}- Lead with the anchors — they shape the day, not the other way around. Put each anchor in at the right time.
...
- ROUTE — this is as important as the stops themselves. Every anchor, find and place above carries lat/lng. Use them. The day must form a coherent path: each stop should be near the one before it, and the traveller must never be sent back past somewhere they have already been. Before you finalise, walk your own list in order and check each consecutive pair. If a stop sits well off the line between its neighbours, either move it beside the stops it is actually closest to, or replace it with a nearer alternative. A slightly less exciting stop that is on the way beats a better one that costs a 20-minute detour each direction — a day that zigzags is a worse day, and the traveller feels it in the car, not on the page.
- Order stops so the day flows outward and back, or along one line — not scattered around the traveller's starting point. Geographic clustering is a hard requirement, not a preference.
...
- HONEST HEDGING: when a stop's time is NOT verified (a venue with unknown hours, or an event with no start time or only "low" confidence), phrase its copy as approximate, set "unverified": true, and add a short "time_note" nudging the user to confirm (e.g. "Racing starts ~6:40pm — confirm before you head out"). When the time IS verified (from a place's hours, a find with a VERIFIED start time, or a "high"-confidence event start time), leave unverified false and omit time_note.
- VERIFIED TIMES: A find marked "VERIFIED start …" is ground truth — schedule the stop to that exact time, state the time plainly (no approximations, no hedging), set "verified": true, and set "verify_source" to that find's exact source URL. Never set both "verified" and "unverified"/"time_note" on the same stop — verified wins.
...
${avoid.length ? `- HARD AVOID: never include any of these places the traveler asked to avoid: ${avoid.join(', ')}. Do not schedule them even to fill a gap.\n` : ''}
- ${p.pace === 'relaxed' ? '4–5' : p.pace === 'packed' ? '7–8' : '5–6'} stops.${
  synthesisConstraint(p.gettingAround)
    ? ' This count YIELDS to the getting-around constraint above — if you cannot find that many stops close enough together, return fewer. A short day they can actually do beats a full day they cannot.'
    : ''
}
...
Each stop MAY also include: time_note (a short "confirm this time" nudge string, present only when the time is approximate/unverified), unverified (boolean — true when the stop's time isn't verified from hours or a high-confidence start time), verified (boolean — true only when scheduled to a confirmed source time), verify_source (the confirming URL string), splurge (boolean — true only on an above-budget splurge stop).`;
...
export function validateStops(raw) {
  ...
  return arr.filter((s) =>
    ...
    ...(s.verified === true && typeof s.verify_source === 'string' && s.verify_source.trim() ? {} : s.time_note ? { time_note: String(s.time_note) } : {}),
    ...(s.verified === true && typeof s.verify_source === 'string' && s.verify_source.trim() ? {} : typeof s.unverified === 'boolean' ? { unverified: s.unverified } : {}),
    ...(s.verified === true && typeof s.verify_source === 'string' && s.verify_source.trim() ? { verified: true } : {}),
    ...(s.verified === true && typeof s.verify_source === 'string' && s.verify_source.trim() ? { verify_source: String(s.verify_source) } : {}),
 ...
 * Build the day.
 *
 * ⚠ THIS IS THE ONLY UNBOUNDED STEP IN A 60-SECOND BOX, which is why it takes a deadline.
 * Vercel kills the function at `maxDuration` (60s, and the Hobby plan's ceiling — we cannot buy
 * headroom). Everything upstream is capped: discovery at 4 searches, the verify phase at 8s,
 * per-item verification at 6s. Only this call could run as long as it liked, and it is also the
 * step that GROWS when live research succeeds — more anchors and finds means a longer prompt and
 * a longer generation. So the runs that died were exactly the ones where research worked, which
 * is the worst possible selection: the best-researched days were the ones nobody ever received.
 *
 * Passing the deadline converts a 504 after sixty seconds into a fallback itinerary at fifty.
 * `deadlineAt` is an absolute epoch ms; when it passes we abort the request and return [], which
 * the callers already treat as "use buildFallbackItinerary" — that path exists and is tested, so
 * nothing new has to be invented for the unhappy case.
 *
 * Aborting rather than racing is deliberate: a raced-and-abandoned call still runs to completion
 * on Anthropic's side and still bills for the tokens we threw away.
 */
export async function runSynthesis({ places, finds, anchors, ctx, deadlineAt = null }) {
  ...
  try {
    if (deadlineAt) {
      ...
      // Research already ate the whole budget. Don't spend an expensive call we cannot use.
      if (remaining <= 0) {
        logUsage({ route: 'synthesis', model: 'synthesis-skipped-no-budget', requests: 1 });
        return [];
      }
    ...
    // Logged as its own row so this is COUNTABLE in production. A deadline that fires silently
    // is indistinguishable from one that never fires, and the whole reason this bug survived is
    // that killed requests left no trace.
    if (timedOut) {
      console.warn('[synthesis] deadline hit — falling back rather than being killed mid-flight');
      logUsage({ route: 'synthesis', model: 'synthesis-deadline', requests: 1 });
      return [];
    }
