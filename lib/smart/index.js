...
import { currentFirecrawlDegraded } from '../usageContext.js';
...
export async function runSmartEngine({ ctx, places, startedAt = null }, deps = {}) {
  ...
  const empty = { itinerary: null, anchors: [], finds: [], hadLiveData: false, liveDataDegraded: null, localHappenings: null };
  try {
    ...
    if (finds.length === 0 && !hasPlaces) return { ...empty, liveDataDegraded: currentFirecrawlDegraded(), localHappenings };
    ...
    const liveDataDegraded = currentFirecrawlDegraded();
    if (!stops.length) return { ...empty, finds, anchors, hadLiveData, liveDataDegraded, localHappenings };
    return { itinerary: stops, anchors, finds, hadLiveData, liveDataDegraded, localHappenings };
  } catch (e) {
    console.error('[smart-engine] unexpected:', e.message);
    return { ...empty, liveDataDegraded: currentFirecrawlDegraded() };
  }
}
